from groq import Groq   # Importting the Groq library to use its API.
...
# A list of system instructions for the AI chatbot.
SystemChatBot = [
...
def RealtimeInformation():
    ...
    date = current_date_time.strftime("%d")  # Get the day of the month.
    month = current_date_time.strftime("%B")  # Get the month name.
    year = current_date_time.strftime("%Y")  # Get the year.
    hour = current_date_time.strftime("%H")  # Get the 24-hour format of the hour.
    minute = current_date_time.strftime("%M")  # Get the minute of the hour.
    second = current_date_time.strftime("%S")  # Get the second of the minute.
...
def ChatBot(Query):
    ...
    try:
        # Koad the existing chat log from the JSON file.
        with open(r"Data\ChatLog.json", "r") as f:
            messages = load(f)
        ...
        for chunk in completion:
            if chunk.choices[0].delta.content:  # Checks if there's content in the current chunk.
